...
+ function($) {
  ...
  var sub = function (data) {
    if (!data.sub||data.sub.length==0) return [{ name: '', code: data.code }];  // 有可能某些县级市没有区
    return format(data.sub);
  };

    var getCities = function (d, mustChoose) {
    for(var i=0;i< raw.length;i++) {
        if (raw[i].code === d || raw[i].name === d) {
            if (mustChoose && raw[i] && raw[i].sub && raw[i].sub[0].name) {
                raw[i].sub.unshift({ name: "", code: "", "sub": [] })
            }
            return sub(raw[i])
        }
    }
      ...
      if(raw[i].code === p || raw[i].name === p) {
        for(var j=0;j< raw[i].sub.length;j++) {
            if (raw[i].sub[j].code === c || raw[i].sub[j].name === c) {
                ...
                var dis = [];
                var bo = false;
                for (var f = 0; f < raw[i].sub[j].sub.length; f++) { 
                    if (raw[i].sub[j].sub[f].name == "" && raw[i].sub[j].sub[f].code == "") {
                        bo = true;
                        break;
                    }
                }
                ...
                if (bo != true) {
                    ...
                    dis.push({ name: "", code: "", sub: [] });
                    for (var f = 0; f < raw[i].sub[j].sub.length; f++) {
                        dis.push(raw[i].sub[j].sub[f]);
                    }
                raw[i].sub[j].sub = dis;
                }
    ...
    return this.each(function() {
      ...
      var provincesCode = raw.map(function(d) {
        ...
        if (params.mustChoose) {
            raw[0].sub.unshift({ name: "", code: "", "sub": [] })
        }
        ...
        formatValue: function (p, values, displayValues) {
          ...
          var newProvince = picker.cols[0].displayValue;
              ...
              if (newProvince !== currentProvince) {
                  var newCities = getCities(newProvince, params.mustChoose);
          ...
          //如果最后一列是空的，那么取倒数第二列
          var len = (values[values.length-1] ? values.length - 1 : (values[values.length-2]?values.length-2: values.length - 3))
      ...
      if (val) {
        ...
        p.value = parseInitValue(val);
          if (p.value[0]) {
              var cities = getCities(p.value[0], params.mustChoose);
  ...
  defaults = $.fn.cityPicker.prototype.defaults = {
      ...
      mustChoose:false//是否显示空白（省市县是否必须选择值）
  };
